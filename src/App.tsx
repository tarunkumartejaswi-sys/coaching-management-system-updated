...
export default function App() {
  ...
  const saveStudentProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (profilePhotoFile) {
        photoUrl = await uploadProfileImage(profilePhotoFile, "student-profile", `students/${studentData.studentId}/profile`);
      }
      const allowed = {
  ...
  const saveDirector = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (directorPhotoFile) photoUrl = await uploadProfileImage(directorPhotoFile, "director", "director/profile");
  ...
  const uploadAcademicFile = async (file: File, folder: string) => {
    const storage = getStorage(auth.app, "gs://coaching-management-system-new.firebasestorage.app");
  ...
  const uploadProfileImage = async (file: File, kind: "student-profile" | "director", path: string) => {
    ...
    const response = await fetch("/api/admin-upload-image", {
      ...
      body: JSON.stringify({ kind, path, contentType: file.type, data: btoa(binary) })
    });
